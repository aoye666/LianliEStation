...
router.post("/publish", (req, res) => {
  const { author_id, title, content, price, campus_id } = req.body;  // 修改字段名称

  // 确保必需的字段存在
  if (!author_id || !title || !campus_id) {
  ...
  return db
    .query(
      ...
      [author_id, title, content, price, campus_id]
    )
    .then(() => {
...
router.get("/byID/:post_id", (req, res) => {
  ...
  db.query("SELECT id, title, content, author_id, created_at, status, price, campus_id FROM posts WHERE id = ? AND status != 'deleted'", [post_id])  // 排除已删除的帖子
    .then(([rows]) => {
      if (rows.length === 0) {
...
router.get("/search", (req, res) => {
  const { title, status, campus_id, min_price, max_price } = req.query;
...
router.put("/:post_id", (req, res) => {
  ...
  const { author_id, title, content, price, campus_id, status } = req.body;
  ...
  db.query("SELECT * FROM posts WHERE id = ? AND author_id = ? AND status != 'deleted'", [post_id, author_id])
    .then(([rows]) => {
      ...
      const updateQuery = `
        UPDATE posts
        SET title = ?, content = ?, price = ?, campus_id = ?, status = ?
        WHERE id = ?`;
      db.query(updateQuery, [title, content, price, campus_id, status, post_id])
        .then(() => {
